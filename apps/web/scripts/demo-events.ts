...
async function main(): Promise<void> {
  ...
  const found = await (await getCatalogProvider()).search("daft punk", { type: "song", limit: 2 });
  const playable = found.tracks.filter((t) => t.isEmbeddable);
  await ingestTracks(playable);
  const [a, b] = playable.map((t) => t.providerTrackId);
  playable.forEach((t) => console.log(`  ${t.providerTrackId}  ${t.title} (${t.durationSec}s)`));

  console.log("\n# post play events: full play of A, quick skip of A, full play of B");
  await postEvents(
    auth(token, "POST", {
      events: [
        { trackId: a, listenedSec: 200, source: "collection", collectionId: "c1", clientHourOfDay: 9 },
        { trackId: a, listenedSec: 4, source: "search", clientHourOfDay: 9 },
        { trackId: b, listenedSec: 180, source: "recommendation", recommendationId: "rec-42", clientHourOfDay: 22 },
      ],
    })
  );

  const state = async (id: string) =>
    (await adminDb().collection("users").doc(uid).collection("trackState").doc(id).get()).data() as TrackState;
  const sa = await state(a);
  const sb = await state(b);
  console.log(`  ${a}: plays=${sa.playCount} completed=${sa.completedCount} skipped=${sa.skipCount} listenedSec=${sa.totalListenedSec}`);
  console.log(`  ${b}: plays=${sb.playCount} completed=${sb.completedCount} skipped=${sb.skipCount} listenedSec=${sb.totalListenedSec}`);
  ...
  const after = await adminDb().collection("playEvents").where("userId", "==", uid).get();
  const saAfter = await state(a);
  console.log(`  events now: ${after.size}; ${a} plays reset to ${saAfter.playCount}`);
