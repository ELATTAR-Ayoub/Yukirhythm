...
async function main(): Promise<void> {
  ...
  const {
    ...
    POST: mePOST,
    PATCH: mePATCH,
  } = await import("@/app/api/me/route");
  const { POST: collPOST } = await import("@/app/api/collections/route");
  const { PUT: addTrack } =
    await import("@/app/api/collections/[collectionId]/tracks/[trackId]/route");
  const { PUT: likeTrack } =
    await import("@/app/api/me/tracks/[trackId]/route");
  ...
  console.log(
    `before: users=${before.users} collections=${before.collections}\n`
  );
  ...
  try {
    ...
    console.log(
      `  stored userId=${me.userId} (matches token uid: ${me.userId === uid}) authProvider=${me.authProvider}`
    );
    ...
    const patchRes = await mePATCH(
      auth(idToken, "PATCH", { privacy: { saveHistory: false } })
    );
    const patched = (await patchRes.json()) as {
      privacy: { saveHistory: boolean };
    };
    ...
    const found = await provider.search("daft punk", {
      type: "song",
      limit: 2,
    });
    ...
    created.artistIds = [
      ...new Set(playable.flatMap((t) => t.artists.map((a) => a.artistId))),
    ];
    ...
    const collRes = await collPOST(
      auth(idToken, "POST", {
        title: "Round Trip",
        trackIds: [created.trackIds[0]],
      })
    );
    const coll = (await collRes.json()) as {
      collectionId: string;
      stats: { trackCount: number };
    };
    created.collectionId = coll.collectionId;
    console.log(
      `POST /api/collections -> ${collRes.status} id=${coll.collectionId} trackCount=${coll.stats.trackCount}`
    );
    ...
    console.log(
      `GET /api/me/likes (virtual=${liked.virtual}): ${liked.tracks.map((t) => t.title).join(" | ")}`
    );
    ...
    for (const id of created.trackIds)
