...
import { PUT as save, DELETE as unsave } from "../collections/[collectionId]/save/route";
...
describe("follow graph against real Firestore", () => {
  ...
  it("writes both edges and both counts, idempotently", async () => {
    await follow(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ uid: uidB }) });
    await follow(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ uid: uidB }) }); // no-op

    const a = (await adminDb().collection("users").doc(uidA).get()).data() as User;
    const b = (await adminDb().collection("users").doc(uidB).get()).data() as User;
    expect(a.counts.followingCount).toBe(1);
    expect(b.counts.followerCount).toBe(1);

    const edge = await adminDb().collection("users").doc(uidB).collection("followers").doc(uidA).get();
    expect(edge.exists).toBe(true);
  });

  it("rejects following yourself", async () => {
    const res = await follow(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ uid: uidA }) });
    expect(res.status).toBe(400);
  });

  it("unfollow removes both edges and decrements", async () => {
    await follow(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ uid: uidB }) });
    await unfollow(auth(tokenA, "/x", "DELETE"), { params: Promise.resolve({ uid: uidB }) });
    const b = (await adminDb().collection("users").doc(uidB).get()).data() as User;
  ...
  it("lists followers", async () => {
    await follow(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ uid: uidB }) });
    const list = (await (await followers(auth(tokenB), { params: Promise.resolve({ uid: uidB }) })).json()) as {
      items: { userId: string }[];
    };
...
describe("public profile against real Firestore", () => {
  ...
  it("404s a private profile even to an authenticated caller", async () => {
    const res = await getProfile(auth(tokenA), { params: Promise.resolve({ uid: uidB }) });
    expect(res.status).toBe(404);
  });

  it("returns a projection without private fields once public", async () => {
    await patchUser(auth(tokenB, "/api/me", "PATCH", { privacy: { publicProfile: true } }));
    const res = await getProfile(auth(tokenA), { params: Promise.resolve({ uid: uidB }) });
    expect(res.status).toBe(200);
    const p = (await res.json()) as PublicProfile & { email?: string; privacy?: unknown };
...
describe("saved collections against real Firestore", () => {
  ...
  beforeEach(async () => {
    ...
    const c = (await (
      await createCollection(auth(tokenB, "/api/collections", "POST", { title: "B's Mix" }))
    ).json()) as Collection;
    publicId = c.collectionId;
    await patchCollection(auth(tokenB, "/x", "PATCH", { visibility: "public" }), {
  ...
  it("A saves B's public collection; saveCount rises and it joins A's library", async () => {
    await save(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ collectionId: publicId }) });

    const c = (await adminDb().collection("collections").doc(publicId).get()).data() as Collection;
    expect(c.stats.saveCount).toBe(1);

    const lib = (await (await listCollections(auth(tokenA))).json()) as Collection[];
    expect(lib.map((x) => x.collectionId)).toContain(publicId);
  });

  it("cannot save your own collection", async () => {
    const res = await save(auth(tokenB, "/x", "PUT"), { params: Promise.resolve({ collectionId: publicId }) });
    expect(res.status).toBe(400);
  });

  it("cannot save a private collection", async () => {
    await patchCollection(auth(tokenB, "/x", "PATCH", { visibility: "private" }), {
      params: Promise.resolve({ collectionId: publicId }),
    });
    const res = await save(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ collectionId: publicId }) });
  ...
  it("a collection turned private drops from the library but the record survives and returns", async () => {
    ...
    let lib = (await (await listCollections(auth(tokenA))).json()) as Collection[];
    expect(lib.map((x) => x.collectionId)).not.toContain(publicId);
    // the saved record is retained
    const rec = await adminDb().collection("users").doc(uidA).collection("savedCollections").doc(publicId).get();
  ...
  it("unsave decrements saveCount", async () => {
    await save(auth(tokenA, "/x", "PUT"), { params: Promise.resolve({ collectionId: publicId }) });
    await unsave(auth(tokenA, "/x", "DELETE"), { params: Promise.resolve({ collectionId: publicId }) });
    const c = (await adminDb().collection("collections").doc(publicId).get()).data() as Collection;
    expect(c.stats.saveCount).toBe(0);
  });

  it("lists another user's public collections, hiding private ones", async () => {
    await createCollection(auth(tokenB, "/api/collections", "POST", { title: "Secret" })); // stays private
    const list = (await (
      await publicCollections(auth(tokenA, `/api/collections/public?ownerId=${uidB}`))
    ).json()) as Collection[];
