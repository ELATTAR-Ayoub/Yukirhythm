...
describe("playback state against real Firestore", () => {
  ...
  it("clamps volume into [0,1] and floors a negative position to 0", async () => {
    const over = (await (
      await PUT(auth(token, "PUT", { volume: 1.5 }))
    ).json()) as PlaybackState;
    expect(over.volume).toBe(1);
    const under = (await (
      await PUT(auth(token, "PUT", { volume: -1, positionSec: -5 }))
    ).json()) as PlaybackState;
  ...
  it("rejects a bogus repeat mode, keeping the previous value", async () => {
    ...
    const s = (await (
      await PUT(auth(token, "PUT", { repeatMode: "sideways" }))
    ).json()) as PlaybackState;
  ...
  it("merges a partial PUT without wiping unset fields", async () => {
    ...
    const s = (await (
      await PUT(auth(token, "PUT", { positionSec: 30 }))
    ).json()) as PlaybackState;
...
describe("queue mutations against real Firestore", () => {
  beforeEach(async () => {
    ...
    await PUT(auth(token, "PUT", { queue: ["t1", "t2", "t3"], queueIndex: 1 }));
  });

  it("play-next pushes onto the manual queue", async () => {
    const s = (await (
      await enqueue(auth(token, "POST", { trackId: "x", mode: "next" }))
    ).json()) as PlaybackState;
  ...
  it("enqueue-end appends to the main queue", async () => {
    const s = (await (
      await enqueue(auth(token, "POST", { trackId: "x", mode: "end" }))
    ).json()) as PlaybackState;
  ...
  it("removing an item before the cursor keeps the current track", async () => {
    ...
    const s = (await (
      await removeAt(auth(token, "DELETE"), {
  ...
  it("removing an item after the cursor leaves the cursor put", async () => {
    const s = (await (
      await removeAt(auth(token, "DELETE"), {
        params: Promise.resolve({ index: "2" }),
      })
    ).json()) as PlaybackState;
  ...
  it("400 on an out-of-range index", async () => {
    expect(
      (
        await removeAt(auth(token, "DELETE"), {
          params: Promise.resolve({ index: "9" }),
        })
      ).status
    ).toBe(400);
  });
