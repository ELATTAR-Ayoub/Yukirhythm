...
export const SPOTIFY_IMPORT_TOKEN_KEY = "yukirhythm:spotify-import:token:v1";
export const SPOTIFY_IMPORT_OAUTH_KEY = "yukirhythm:spotify-import:oauth:v1";
...
function spotifyErrorMessage(
  ...
  if (status === 403)
    return "Spotify only allows importing playlists you own or collaborate on.";
  if (status === 429)
...
async function spotifyRequest<T>(
  ...
  if (!response.ok) {
    ...
    throw new SpotifyImportError(
      spotifyErrorMessage(response.status, response.statusText, detail),
      response.status,
